...
const UsersTrash = () => {
  ...
  const fetchTrash = async () => {
    ...
    setError('');
    try {
      const response = await api.get('/api/users/trash/list.php');
  ...
  const handleRestore = async (userId) => {
    if (!window.confirm(`Restore user ${userId}?`)) return;

    setActionLoading(`restore-${userId}`);
    try {
      const response = await api.post('/api/users/trash/restore.php', {
        id: userId,
      });
  ...
  const handleRestoreAll = async () => {
    if (!window.confirm('Restore all users from trash?')) return;
    ...
    try {
      ...
      if (response.data.success) {
        setSuccessMessage(`Successfully restored ${response.data.count} user(s)`);
        fetchTrash(); // Refresh list
      } else {
        setError(response.data.message || 'Failed to restore users');
      }
    } catch (err) {
      setError(
        err.response?.data?.message ||
  ...
  const handlePermanentDelete = async (userId) => {
    if (!window.confirm(`Permanently delete user ${userId}? This action cannot be undone!`)) return;

    setActionLoading(`delete-${userId}`);
    try {
      const response = await api.post('/api/users/trash/delete_permanent.php', {
        id: userId,
      });
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm('Permanently delete ALL users from trash? This action cannot be undone!')) return;
    ...
    try {
      ...
      if (response.data.success) {
        setSuccessMessage(`Successfully deleted ${response.data.count} user(s)`);
        fetchTrash(); // Refresh list
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Deleted users (can be restored)</p>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">{user.role?.name || user.role || 'N/A'}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(user.deletedAt)}</td>
